...
int applysizehints(Client *c, int *x, int *y, int *w, int *h, int interact) {
    ...
    *h = MAX(1, *h);
    ...
    if (*w < bh)
        *w = bh;
    if (c->isfloating != 0 || !c->mon->lt[c->mon->sellt]->arrange) {
        if (!c->hintsvalid)
            updatesizehints(c);
...
void arrangemon(Monitor *m) {
    ...
    XMoveWindow(dpy, m->tagwin, m->wx + m->gappov,
                m->by + (m->topbar ? (bh + m->gappoh) : (-(m->mh / scalepreview) - m->gappoh)));
...
void drawbar(Monitor *m) {
    ...
    for (i = 0; i < LENGTH(tags); i++) {
        ...
        if ((m->tagset[m->seltags] &
             1 << i) != 0) /* if there are conflicts, just move these lines directly
...
drawtab(Monitor *m) {
    ...
    for (c = m->clients; c; c = c->next) {
        ...
        drw_text(drw, x + horizpadtabo / 2, vertpadbar / 2, w - horizpadtabo, th - vertpadbar, horizpadtabi / 2,
                 c->name, 0);
        x += w;
        ++i;
    }
...
placemouse(const Arg *arg) {
    ...
    if (!(c = selmon->sel) ||
        !c->mon->lt[c->mon->sellt]->arrange) /* no support for placemouse when floating layout is used */
        return;
...
setnumdesktops(void) {
    ...
    XChangeProperty(dpy, root, netatom[NetNumberOfDesktops], XA_CARDINAL, 32, PropModeReplace, (unsigned char *) data,
                    1);
}
...
void updatebars(void) {
    ...
    for (m = mons; m; m = m->next) {
        ...
        m->tabwin = XCreateWindow(dpy, root, m->wx + m->gappov, m->ty, m->ww - 2 * m->gappov, th, 0,
